"""
train_full_model.py
Trains rizal-xlm-r-full using TSDAE on the FULL-TEXT corpus only.
Corpus: fullversion_noli.csv + fullversion_elfili.csv
Output: backend/model_output/rizal-xlm-r-full/
"""
import os
import sys
import pandas as pd
...
def train_full_model():
    ...
    output_path = os.path.join(base_dir, 'model_output', 'rizal-xlm-r-full')
    os.makedirs(output_path, exist_ok=True)

    # Only full-text files
    full_files = ['fullversion_noli.csv', 'fullversion_elfili.csv']

    train_sentences = []
    logging.info(f"Loading FULL-TEXT corpus from {csv_dir}...")
    ...
    logging.info(f"Total unique FULL-TEXT sentences: {len(train_sentences)}")
    ...
    logging.info("Starting FULL-TEXT model training (3 epochs)...")
    model.fit(
    ...
    logging.info(f"SUCCESS: FULL model saved to {output_path}")
    logging.info("Next: copy/rename to backend/app/models/rizal-xlm-r-full")
    logging.info("Then run: poetry run python scripts/seed_full_embeddings.py")

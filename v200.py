...
class DAPTQueryValidator:
    """
    Domain-Adaptive Pre-Training (DAPT) Query Validator
    Adapts query embeddings using CSV domain corpus for validation only
    """
    
    def __init__(self, model, corpus_texts, similarity_threshold=0.70, debug_mode=False):
        """
        Initialize DAPT validator
        
        Args:
            model: Pre-trained SentenceTransformer model
            corpus_texts: List of texts from CSV domain corpus
            similarity_threshold: Minimum average similarity for query acceptance
            debug_mode: Enable visualization for debugging
        """
        ...
        self.console.print("Building DAPT domain corpus for query validation...", style="cyan")
        
        # Build domain vocabulary and embeddings
        ...
        for text in corpus_texts:
            ...
            self.domain_texts.append(text)
        
        # Compute domain corpus embeddings (for adaptation reference)
        self.console.print(f"Computing domain embeddings for {len(self.domain_texts)} corpus texts...", style="cyan")
        self.domain_embeddings = self.model.encode(
            self.domain_texts[:5000],  # Limit for efficiency
            show_progress_bar=False,
            batch_size=32
        )
        
        # Compute centroid for domain adaptation
        self.domain_centroid = np.mean(self.domain_embeddings, axis=0)
        
        self.console.print(f"DAPT validator ready: {len(self.domain_vocabulary)} domain words", style="green")
    
    def _adapt_embedding(self, original_embedding, adaptation_strength=0.3):
        """
        Adapt embedding towards domain centroid
        This simulates domain-adaptive pre-training effect
        
        Args:
            original_embedding: Original embedding from pre-trained model
            adaptation_strength: How much to shift towards domain (0-1)
        
        Returns:
            Adapted embedding
        """
        # Shift embedding towards domain centroid
        adapted = (1 - adaptation_strength) * original_embedding + adaptation_strength * self.domain_centroid
    ...
    def validate_query(self, query):
        """
        Validate query using DAPT-adapted embeddings
        
        Returns:
            is_valid (bool), validation_info (dict)
        """
        # Tokenize query
        tokens = extract_words(query)
        
        if not tokens:
            return False, {
                'reason': 'No valid tokens in query',
                'tokens': [],
                'avg_similarity': 0.0,
        ...
        if len(tokens) == 1:
            # Single token - check domain vocabulary presence
            in_domain = tokens[0].lower() in self.domain_vocabulary
            return in_domain, {
                'reason': 'Single token query' if in_domain else 'Token not in domain vocabulary',
                'tokens': tokens,
                'avg_similarity': 1.0 if in_domain else 0.0,
                'threshold': self.similarity_threshold,
                'passed': in_domain
            }
        
        # Step 1: Generate original embeddings
        original_embeddings = self.model.encode(tokens, show_progress_bar=False)
        
        # Step 2: Adapt embeddings using DAPT
        adapted_embeddings = np.array([
            self._adapt_embedding(emb) for emb in original_embeddings
        ])
        
        # Step 3: Compute pairwise cosine similarity
        similarity_matrix = cosine_similarity(adapted_embeddings)
        
        # Step 4: Calculate average similarity (excluding diagonal)
        n = len(tokens)
        if n > 1:
        ...
        validation_info = {
            ...
            'threshold': self.similarity_threshold,
            'passed': is_valid,
            'reason': 'Query tokens are contextually coherent' if is_valid else 'Query tokens lack contextual coherence',
        ...
        if self.debug_mode:
            self._visualize_validation(validation_info)
        
        return is_valid, validation_info
    
    def _visualize_validation(self, validation_info):
        """Generate visualization for debugging"""
        tokens = validation_info['tokens']
        similarity_matrix = validation_info['similarity_matrix']
        adapted_embeddings = validation_info['adapted_embeddings']
        ...
        ax1.set_title('DAPT-Adapted Token Similarity Matrix')
        
        # PCA scatter
        if len(adapted_embeddings) > 2:
            pca = PCA(n_components=2)
            reduced = pca.fit_transform(adapted_embeddings)
            
            ax2.scatter(reduced[:, 0], reduced[:, 1], s=100, c='blue', alpha=0.6)
            for i, token in enumerate(tokens):
                ax2.annotate(token, (reduced[i, 0], reduced[i, 1]), fontsize=10)
            
            ax2.set_title('PCA Projection of Adapted Embeddings')
            ax2.set_xlabel(f'PC1 ({pca.explained_variance_ratio_[0]:.1%})')
            ax2.set_ylabel(f'PC2 ({pca.explained_variance_ratio_[1]:.1%})')
        else:
            ax2.text(0.5, 0.5, 'Need >2 tokens for PCA', ha='center', va='center')
            ax2.set_title('PCA Projection (Insufficient Tokens)')
        
        plt.tight_layout()
        plt.savefig('dapt_validation_debug.png', dpi=150, bbox_inches='tight')
        plt.close()
        
        self.console.print("[dim]Debug visualization saved to dapt_validation_debug.png[/dim]")
...
class CleanNoliSystem:
    """
    Dynamic Dual-Formula CLEAR System with DAPT Query Validation
    - DAPT: Query validation only (domain-adapted embeddings)
    - Main Processing: Original pre-trained model (unchanged)
    """

    def __init__(self, dapt_threshold=0.70, dapt_debug=False):
        self.console = Console()
        ...
        # Initialize DAPT Query Validator
        self.console.print("Initializing DAPT Query Validator...", style="cyan")
        corpus_texts = self._get_all_corpus_texts()
        self.dapt_validator = DAPTQueryValidator(
            model=self.model,
            corpus_texts=corpus_texts,
            similarity_threshold=dapt_threshold,
            debug_mode=dapt_debug
        )
        ...
        self.console.print("Dynamic Dual-Formula CLEAR system with DAPT ready!", style="bold green")
    ...
    def _get_all_corpus_texts(self):
        """Extract all texts from corpus for DAPT"""
        corpus_texts = []
        
        for book_data in self.books_data.values():
            # Add sentence texts
            corpus_texts.extend(book_data['chapters']['sentence_text'].astype(str).tolist())
            
            # Add theme meanings
            corpus_texts.extend(book_data['themes']['Meaning'].astype(str).tolist())
        
        return corpus_texts

    def _compute_all_embeddings(self):
        """Compute embeddings for all books using ORIGINAL model"""
        for book_data in self.books_data.values():
    ...
    def _compute_neighbor_similarity(self, main_text, main_length, neighbor_text, neighbor_length):
        ...
        # Semantic similarity (ORIGINAL MODEL)
    ...
    def _retrieve_passages(self, query, query_analysis, book_key, top_k=9):
        """CLEAR-based hybrid retrieval with dynamic length-based weights (ORIGINAL MODEL)"""
        self.used_passages[book_key] = set()
        ...
        # Use ORIGINAL MODEL for query embedding
    ...
    def _get_thematic_classification(self, passages, query, book_key):
        """Classify passages by themes with dynamic length-based weights (ORIGINAL MODEL)"""
        if not passages:
    ...
    def query(self, user_query):
        """Main query interface with DAPT validation and dynamic dual-formula scoring"""
        
        # ============================================
        # PHASE 0: DAPT QUERY VALIDATION (NEW!)
        # ============================================
        is_dapt_valid, dapt_info = self.dapt_validator.validate_query(user_query)
        
        if not is_dapt_valid:
            return {
                'type': 'dapt_validation_failed',
                'dapt_info': dapt_info,
                'message': f"Query validation failed: {dapt_info['reason']}"
            }
        
        # Phase 1: Query Validation (Filipino words)
    ...
    def _display_dapt_validation(self, dapt_info):
        """Display DAPT validation results"""
        tokens = dapt_info['tokens']
        avg_sim = dapt_info['avg_similarity']
        threshold = dapt_info['threshold']
        passed = dapt_info['passed']
        
        status_style = "green" if passed else "red"
        status_text = "✓ PASSED" if passed else "✗ FAILED"
        
        dapt_table = Table(
            title="DAPT Query Validation",
            show_header=True,
            header_style=f"bold {status_style}",
            border_style=status_style,
            box=box.ROUNDED,
            expand=True
        )
        
        dapt_table.add_column("Metric", style="bright_white", width=25)
        dapt_table.add_column("Value", style=status_style, width=40)
        
        dapt_table.add_row("Tokens", ", ".join(tokens))
        dapt_table.add_row("Avg Token Similarity", f"{avg_sim:.3f}")
        dapt_table.add_row("Threshold", f"{threshold:.3f}")
        dapt_table.add_row("Status", status_text)
        dapt_table.add_row("Reason", dapt_info['reason'])
        
        self.console.print("\n")
        self.console.print(dapt_table)
        self.console.print("\n")

    def display_results(self, response, query=""):
        """Display results with DAPT validation and dynamic weight information"""
        result_type = response['type']
        
        # Display DAPT validation if present
        if 'dapt_info' in response:
            self._display_dapt_validation(response['dapt_info'])
        
        # Handle DAPT validation failure
        if result_type == 'dapt_validation_failed':
            dapt_info = response['dapt_info']
            ...
            dapt_panel = Panel(
                f"{response['message']}\n\n"
                f"DAPT Validation Details:\n"
                f"  Tokens: {', '.join(dapt_info['tokens'])}\n"
                f"  Avg Similarity: {dapt_info['avg_similarity']:.3f}\n"
                f"  Threshold: {dapt_info['threshold']:.3f}\n"
                f"  Status: REJECTED\n\n"
                f"The query tokens lack contextual coherence within the domain corpus.",
                title="❌ DAPT Validation Failed",
                style="red",
                box=box.ROUNDED
            )
            self.console.print(dapt_panel)
            return
        ...
        for book_key in ['noli', 'elfili']:
            ...
            metrics_text = (
                f"🔍 DYNAMIC DUAL-FORMULA CLEAR SYSTEM + DAPT 🔍\n"
                f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
                f"DAPT: Query validation with domain-adapted embeddings\n"

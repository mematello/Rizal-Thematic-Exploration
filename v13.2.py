...
class CleanNoliSystem:
    ...
    def query(self, user_query):
        """Main query interface - searches both books with per-book lexical grounding check"""
        ...
        # Get content words for lexical grounding
        ...
        # Check each book separately
        for book_key in self.books_data.keys():
            # Per-book lexical grounding check
            vocab = self.corpus_vocabulary[book_key]
            book_matches = [w for w in content_words if w in vocab]

            # If no lexical grounding for this book, skip retrieval
            if not book_matches:
                no_grounding_books.append(book_key)
                continue

            # Proceed with retrieval for this book
    ...
    def display_results(self, response, query=""):
        ...
        if result_type == 'no_lexical_grounding':
            ...
            # Determine message based on which books have no grounding
        ...
        if 'overlap_info' in response:
            ...
            # Show warnings for books without grounding
            ...
            # Show grounding info for matched books

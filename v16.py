...
class CleanNoliSystem:
    ...
    def _build_corpus_vocabulary(self):
        ...
        for book_key, book_data in self.books_data.items():
            ...
            for text in book_data['themes']['Tagalog Title'].astype(str):
                vocabulary.update(extract_words(text.lower()))
    ...
    def _compute_dynamic_weights(self, query_length, stopword_ratio):
        """Dynamic weighting for main retrieval"""
        if query_length <= 1:
    ...
    def _calculate_neighbor_score(self, semantic_sim, lexical_sim):
        ...
        return 0.65 * semantic_sim + 0.35 * lexical_sim

    def _compute_lexical_score(self, query, sentence_text):
        """Compute lexical overlap score with exact word boundary matching"""
        ...
        query_words = set(w.lower() for w in extract_words(query) if w.lower() not in self.query_analyzer.STOPWORDS)
        ...
        overlap_score = len(matched_words) / len(query_words)
        ...
        if stopword_ratio > self.HIGH_STOPWORD_RATIO:
            ...
            overlap_score *= (1.0 - penalty)

        return overlap_score

    def _calculate_clear_score(self, semantic_sim, lexical_score, weights, sentence_text):
        """CLEAR hybrid scoring with short sentence penalty"""
        lambda_emb, lambda_lex = weights
        final_score = (lambda_emb * semantic_sim) + (lambda_lex * lexical_score)

        # Apply short sentence penalty
    ...
    def _compute_neighbor_similarities(self, main_text, neighbor_text):
        ...
        main_words = set(extract_words(main_text.lower()))
        neighbor_words = set(extract_words(neighbor_text.lower()))

        if main_words and neighbor_words:
    ...
    def _get_expanded_context(self, chapter_num, sentence_num, book_key, main_sentence_text):
    ...
    def _retrieve_passages(self, query, book_key, top_k=9):
        ...
        for idx, semantic_sim in enumerate(semantic_similarities):
            ...
            final_score = self._calculate_clear_score(
                semantic_sim, lexical_score, weights, row['sentence_text']
            )
        ...
        for candidate in candidates:
            ...
            for sent in context['prev_sentences']:
                self.used_passages[book_key].add(
                    self._get_passage_id(ch_num, sent['sentence_number'])
                )
            for sent in context['next_sentences']:
                self.used_passages[book_key].add(
                    self._get_passage_id(ch_num, sent['sentence_number'])
                )
    ...
    def _get_thematic_classification(self, passages, book_key):
    ...
    def query(self, user_query):
        ...
        # Compute global vocabulary on-demand
        global_vocabulary = set()
        for vocab in self.corpus_vocabulary.values():
            global_vocabulary.update(vocab)

        missing_words = [w for w in content_words if w not in global_vocabulary]
        if missing_words:
            return {
                'type': 'no_lexical_grounding',
                'overlap_info': {
        ...
        for book_key in self.books_data.keys():
            ...
            passages = self._retrieve_passages(user_query, book_key)
    ...
    def display_results(self, response, query=""):
        ...
        for book_key in ['noli', 'elfili']:
            ...
            context_count = sum(1 for r in results if r['context']['prev_sentences'] or r['context']['next_sentences'])
            total_context = sum(len(r['context']['prev_sentences']) + len(r['context']['next_sentences']) for r in results)
            ...
            for i, result in enumerate(results, 1):
                ...
                context_count = len(result['context']['prev_sentences']) + len(result['context']['next_sentences'])

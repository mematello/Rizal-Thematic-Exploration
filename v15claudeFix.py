...
if __name__ == "__main__":
    ...
    while True:
        ...
        is_valid, validation_info = analyzer.validate_filipino_query(user_input)
        if not is_valid:
            warning_panel = Panel(
                "Query may not be in Filipino. Results could be unreliable.",
                title="Warning",
                style="yellow",
                box=box.ROUNDED
            )
            system.console.print(warning_panel)
...
class CleanNoliSystem:
    ...
    def _build_corpus_vocabulary(self):
        ...
        for book_key, book_data in self.books_data.items():
            ...
            for text_col in ['sentence_text']:
                for text in book_data['chapters'][text_col].astype(str):
                    vocabulary.update(extract_words(text.lower()))

            for text_col in ['Tagalog Title', 'Meaning']:
                for text in book_data['themes'][text_col].astype(str):
                    vocabulary.update(extract_words(text.lower()))
